...
class ICStartSubtasksQA implements CannedScript{
    ComponentManager componentManager = ComponentManager.getInstance()
    Category log = Category.getInstance(ICStartSubtasks.class)
    
    String getName() {
        return "Allow QA to start resolving subtasks"
    }

    String getDescription() {
        return """Workflow transition "Ready To QA" on all sub-tasks, effectively allowing QA to begin<br>
        """
    }
    ...
    Map doScript(Map params) {
        ...
        if (subTaskManager.subTasksEnabled && !subTasks.empty) {
            subTasks.each {
                ...
                workflowTransitionUtil.setAction (761)    // IC Ready To QA
