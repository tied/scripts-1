...
class ICCopyRoleToCFParent implements CannedScript{
    ComponentManager componentManager = ComponentManager.getInstance()
    Category log = Category.getInstance(ICCopyRoleToCFParent.class)
    public final static String FIELD_ISSUE_ID = "FIELD_ISSUE_ID"
    public final static String FIELD_ROLE_ID = "FIELD_ROLE_ID"
    
    String getName() {
        return "Assign parent to role"
    }

    String getDescription() {
        return """Update a custom-field value with the text name of a given role in the context of the current task's parent task.<br>
        """
    }
    ...
    Map doScript(Map params) {
        ...
        if (issue.isSubTask()) {
            ...
            parent.setCustomFieldValue(cf, role.name)
            componentManager.getIssueManager().updateIssue(user, parent, EventDispatchOption.DO_NOT_DISPATCH, false)
            
            log.debug("Going to reindex");
            componentManager.getIndexManager().reIndex(parent);
            log.debug("Reindexexed " + parent.getKey())
        }     else {
